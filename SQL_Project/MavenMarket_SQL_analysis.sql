...
select p.product_brand, (sum(t.quantity * p.product_retail_price)- sum(t.quantity * p.product_cost) - sum(r.quantity*p.product_retail_price) ) / ( sum(t.quantity * p.product_retail_price) - sum(r.quantity * p.product_retail_price) ) * 100 as Profit_Margin
from MavenMarket_Transactions t
join MavenMarket_Returns r ON t.product_id = r.product_id
join MavenMarket_Products p ON t.product_id = p.product_id
group by p.product_brand;
...
select p.product_name,  ( sum(t.quantity * p.product_retail_price)- -sum(t.quantity * p.product_cost ) - sum(r.quantity * p.product_retail_price ))  / ( sum(t.quantity * p.product_retail_price)-sum(r.quantity * p.product_retail_price )) * 100  as Profit_Margin
from MavenMarket_Transactions t
join MavenMarket_Returns r ON t.product_id = r.product_id
join MavenMarket_Products p ON t.product_id = p.product_id
group by p.product_name
